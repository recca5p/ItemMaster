...
public class SnowflakeRepositoryMethodTests
{
    ...
    public static IEnumerable<object[]> GetItemsBySkusBatchData()
    {
        ...
        yield return new object[]
            { Enumerable.Range(1, 100).Select(i => $"SKU-{i:D3}").ToArray(), 100, "Batch of 100" };
        yield return new object[]
            { Enumerable.Range(1, 1000).Select(i => $"SKU-{i:D4}").ToArray(), 1000, "Exact batch size" };
        yield return new object[]
            { Enumerable.Range(1, 1500).Select(i => $"SKU-{i:D4}").ToArray(), 1500, "Multiple batches" };
    }
        ...
        string[] skus, int expectedCount, string scenario)
    {
        ...
        mockQueryBuilder.Setup(x => x.BuildSelectBySkus(It.IsAny<IEnumerable<string>>()))
            ...
            {
                var sql =
                    $"SELECT * FROM TEST_DB.TEST_SCHEMA.ITEMS WHERE SKU IN ({string.Join(",", inputSkus.Select(s => $"'{s}'"))})";
                return (sql, null);
            });
