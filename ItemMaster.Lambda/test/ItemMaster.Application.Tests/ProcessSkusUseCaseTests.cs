...
public class ProcessSkusUseCaseTests
{
  ...
  public async Task ExecuteAsync_WithValidRequest_ShouldReturnSuccess()
  {
    ...
    _mockOrchestrator.Setup(x => x.ProcessSkusAsync(It.IsAny<ProcessSkusRequest>(), It.IsAny<RequestSource>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
  ...
  public async Task ExecuteAsync_WithCustomTraceId_ShouldUseProvidedId()
  {
    ...
    var expectedResponse = new ProcessSkusResponse();
    _mockOrchestrator.Setup(x => x.ProcessSkusAsync(It.IsAny<ProcessSkusRequest>(), It.IsAny<RequestSource>(), customTraceId, It.IsAny<CancellationToken>()))
        .ReturnsAsync(Result<ProcessSkusResponse>.Success(expectedResponse));
    ...
    // Assert
    result.IsSuccess.Should().BeTrue();
    _mockOrchestrator.Verify(x => x.ProcessSkusAsync(request, RequestSource.Sqs, customTraceId, It.IsAny<CancellationToken>()), Times.Once);
  }
  ...
  public async Task ExecuteAsync_WithNullTraceId_ShouldGenerateNewId()
  {
    ...
    // Act
    var result = await _useCase.ExecuteAsync(request, RequestSource.ApiGateway, null);

    // Assert
    result.IsSuccess.Should().BeTrue();
    _mockOrchestrator.Verify(x => x.ProcessSkusAsync(It.IsAny<ProcessSkusRequest>(), It.IsAny<RequestSource>(), It.Is<string>(s => !string.IsNullOrEmpty(s)), It.IsAny<CancellationToken>()), Times.Once);
  }
  ...
  public async Task ExecuteAsync_WithCancellationToken_ShouldPassItThrough()
  {
    ...
    // Assert
    result.IsSuccess.Should().BeTrue();
    _mockOrchestrator.Verify(x => x.ProcessSkusAsync(It.IsAny<ProcessSkusRequest>(), It.IsAny<RequestSource>(), It.IsAny<string>(), cts.Token), Times.Once);
  }
  ...
  public async Task ExecuteAsync_WithDifferentRequestSources_ShouldPassCorrectSource(RequestSource source)
  {
    ...
    var request = new ProcessSkusRequest { Skus = new List<string> { "TEST-001" } };
    var expectedResponse = new ProcessSkusResponse();
    _mockOrchestrator.Setup(x => x.ProcessSkusAsync(It.IsAny<ProcessSkusRequest>(), source, It.IsAny<string>(), It.IsAny<CancellationToken>()))
        .ReturnsAsync(Result<ProcessSkusResponse>.Success(expectedResponse));
    ...
    // Assert
    result.IsSuccess.Should().BeTrue();
    _mockOrchestrator.Verify(x => x.ProcessSkusAsync(request, source, It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);
  }
}
