...
{
    Task<Result<ProcessSkusResponse>> ProcessSkusAsync(ProcessSkusRequest request, RequestSource requestSource, string traceId, CancellationToken cancellationToken);
}
...
public class SkuProcessingOrchestrator : ISkuProcessingOrchestrator
{
    ...
    private const string HEALTH_CHECK_LOG_MESSAGE = "Health check request with no SKUs - returning success without processing | TraceId: {TraceId}";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (IsHealthCheckRequest(requestedSkus, requestSource))
        {
            return CreateHealthCheckResponse(traceId);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await _itemPublishingService.PublishItemsAsync(mappingResult.UnifiedItems, requestSource, traceId, cancellationToken);
    ...
    private async Task<List<Domain.Item>> FetchItemsAsync(
        List<string> requestedSkus, 
        RequestSource requestSource, 
        string traceId, 
        CancellationToken cancellationToken)
    {
        ...
        {
            return await _itemFetchingService.FetchItemsBySkusAsync(requestedSkus, requestSource, traceId, cancellationToken);
        }
    ...
    private void LogProcessingSummary(List<Domain.Item> itemsList, List<string> notFoundSkus, ItemMappingResult mappingResult, string traceId)
    {
        _logger.LogInformation(
            "PROCESSING_COMPLETE | Found: {Found} | NotFound: {NotFound} | Mapped: {Mapped} | Skipped: {Skipped} | TraceId: {TraceId}",
            itemsList.Count, notFoundSkus.Count, mappingResult.UnifiedItems.Count, mappingResult.SkippedItems.Count, traceId);
    }
