...
public class CachedSecretService : ICachedSecretService
{
    ...
    private async Task<string> FetchAndCacheSecretAsync(string secretKey, string cacheKey, Func<Task<string>> secretRetriever)
    {
    ...
    private static string GenerateCacheKey(string secretKey) => $"{CACHE_KEY_PREFIX}{secretKey}";
